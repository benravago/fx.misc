...
class LineSelection<PS, SEG, S> extends SelectionBase<PS, SEG, S> {
  ...
  LineSelection(GenericStyledArea<PS, SEG, S> area, ObjectProperty<Paint> lineHighlighterFill) {
    super("line-highlighter", area, path -> {
      if (lineHighlighterFill == null) {
        ...
        path.highlightFillProperty().bind(lineHighlighterFill);
      }
      path.getElements().addListener((Change<? extends PathElement> chg) -> {
        if (chg.next() && chg.wasAdded() || chg.wasReplaced()) {
          var width = path.getParent().getLayoutBounds().getWidth();
  ...
  void selectCurrentLine() {
    var p = getArea().getCurrentParagraph();
    var start = getArea().getAbsolutePosition(p, getArea().getCurrentLineStartInParargraph());
    var end = getArea().getAbsolutePosition(p, getArea().getCurrentLineEndInParargraph());
