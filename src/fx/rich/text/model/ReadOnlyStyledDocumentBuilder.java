...
public final class ReadOnlyStyledDocumentBuilder<PS, SEG, S> {
  ...
  public static <PS, SEG, S> ReadOnlyStyledDocument<PS, SEG, S> constructDocument(SegmentOps<SEG, S> segmentOps, PS defaultParagraphStyle, Consumer<ReadOnlyStyledDocumentBuilder<PS, SEG, S>> configuration) {
    var builder = new ReadOnlyStyledDocumentBuilder<PS, SEG, S>(segmentOps, defaultParagraphStyle);
  ...
  public static <PS, SEG, S> ReadOnlyStyledDocument<PS, SEG, S> constructDocument(SegmentOps<SEG, S> segmentOps, PS defaultParagraphStyle, int initialCapacity, Consumer<ReadOnlyStyledDocumentBuilder<PS, SEG, S>> configuration) {
    var builder = new ReadOnlyStyledDocumentBuilder<PS, SEG, S>(segmentOps, defaultParagraphStyle, initialCapacity);
    configuration.accept(builder);
    return builder.build();
  }

  final SegmentOps<SEG, S> segmentOps;
  final PS defaultParagraphStyle;
  final List<Paragraph<PS, SEG, S>> paragraphList;
  ...
  ReadOnlyStyledDocumentBuilder(SegmentOps<SEG, S> segmentOps, PS defaultParagraphStyle, List<Paragraph<PS, SEG, S>> list) {
  ...
  <T> ReadOnlyStyledDocumentBuilder<PS, SEG, S> addParagraphList(List<T> paragraphContentList, StyleSpans<S> spansThroughoutDocument, Function<T, PS> getStyle, Function<T, List<SEG>> getSegList) {
    var docLength = paragraphContentList.stream()
      .map(getSegList)
    ...
    var styleOffset = 0;
    for (var paragraphContent : paragraphContentList) {
      var paragraphStyle = argumentOrDefault(getStyle.apply(paragraphContent));
      var segList = getSegList.apply(paragraphContent);
      var paragraphLength = segList.stream().mapToInt(segmentOps::length).sum();
      var styleEnd = styleOffset + paragraphLength;
      var spans = spansThroughoutDocument.subView(styleOffset, styleEnd);
   ...
   */
  PS argumentOrDefault(PS paragraphStyle) {
    return paragraphStyle != null ? paragraphStyle : defaultParagraphStyle;
  }

  void ensureNotYetCreated() {
    if (alreadyCreated) {
      throw new IllegalStateException(
        "This builder has already been used to create a list of Paragraphs. "
        + "One builder can only be used to build a single list. To create a new one, create a new builder");
    }
  }

  ReadOnlyStyledDocumentBuilder<PS, SEG, S> addPar(Paragraph<PS, SEG, S> paragraph) {
    ensureNotYetCreated();
