...
public interface StyleSpans<S> extends Iterable<StyleSpan<S>>, TwoDimensional {
  ...
  default StyleSpans<S> append(StyleSpan<S> span) {
    ...
    var lastIdx = getSpanCount() - 1;
    var myLastSpan = getStyleSpan(lastIdx);
    if (Objects.equals(myLastSpan.getStyle(), span.getStyle())) {
      var newLastSpan = new StyleSpan<>(span.getStyle(), myLastSpan.getLength() + span.getLength());
  ...
  default StyleSpans<S> prepend(StyleSpan<S> span) {
    ...
    var myFirstSpan = getStyleSpan(0);
    if (Objects.equals(span.getStyle(), myFirstSpan.getStyle())) {
      var newFirstSpan = new StyleSpan<>(span.getStyle(), span.getLength() + myFirstSpan.getLength());
  ...
  default StyleSpans<S> subView(int from, int to) {
    ...
    var end = to > from ? start.offsetBy(to - from, Backward) : start;
  ...
  default StyleSpans<S> concat(StyleSpans<S> that) {
    ...
    var n1 = this.getSpanCount();
    var n2 = that.getSpanCount();

    var myLast = this.getStyleSpan(n1 - 1);
    var theirFirst = that.getStyleSpan(0);
  ...
  default StyleSpans<S> mapStyles(UnaryOperator<S> mapper) {
    var builder = new StyleSpansBuilder<S>(getSpanCount());
    for (var span : this) {
  ...
  default Stream<StyleSpan<S>> stream() {
    var spliterator = new Spliterator<StyleSpan<S>>() {
      final Iterator<StyleSpan<S>> iterator = iterator();
