...
/**
 * An object that specifies where a change occurred in a {@link fx.rich.text.GenericStyledArea}.
 */
public class RichTextChange<PS, SEG, S> extends TextChange<StyledDocument<PS, SEG, S>, RichTextChange<PS, SEG, S>> {
  ...
  static boolean skipStyleComparison = false;
  ...
  public boolean equals(Object other) {
    if (skipStyleComparison && other instanceof RichTextChange<?,?,?> rtc) {
      var otherChange = rtc.toPlainTextChange();
      boolean matches = toPlainTextChange().equals(otherChange);
      if (!matches) {
        System.err.println("Plain text comparison mismatch caused by text change"
