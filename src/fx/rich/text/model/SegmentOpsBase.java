...
public abstract class SegmentOpsBase<SEG, S> implements SegmentOps<SEG, S> {
  ...
  public final SEG subSequence(SEG seg, int start, int end) {
    ...
    if (end > length(seg)) {
      throw new IllegalArgumentException(String.format("End cannot be greater than segment's length. End=%s Length=%s", end, length(seg)));
    }
    return length(seg) == 0 || start == end ? empty : realSubSequence(seg, start, end);
  }
  ...
  public final SEG subSequence(SEG seg, int start) {
    return length(seg) == 0 || start == length(seg) ? empty : realSubSequence(seg, start);
  }
