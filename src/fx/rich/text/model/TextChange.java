 ...
 * in a {@link fx.rich.text.TextEditingArea}.
 *
 * @param <S> type of data that was removed and inserted in the {@link fx.rich.text.TextEditingArea}.
...
public abstract class TextChange<S, Self extends TextChange<S, Self>> {
   ...
   */
  public Self invert() {
    return create(position, inserted, removed);
  }
  ...
  public Optional<Self> mergeWith(Self latter) {
    if (latter.position == this.position + this.insertedLength()) {
      var removedText = concat(this.removed, latter.removed);
      var addedText = concat(this.inserted, latter.inserted);
      ...
      if (this.position <= latter.position) {
        var addedText = concat(sub(this.inserted, 0, latter.position - this.position), latter.inserted);
        return Optional.of(create(this.position, this.removed, addedText));
      } else {
        var removedText = concat(sub(latter.removed, 0, this.position - latter.position), this.removed);
  ...
  public boolean equals(Object other) {
    return (other instanceof TextChange<?, ?> that)
