...
public class SelectionPath extends Path {
  ...
  public final void setHighlightFill(Paint paint) {
    highlightFill.set(paint);
  }

  final Val<IndexRange> range;

  final Val<IndexRange> rangeProperty() {
    return range;
  }
  ...
  static final CssMetaData<SelectionPath, Paint> HIGHLIGHT_FILL =
    new CustomCssMetaData<>("-fx-highlight-fill", StyleConverter.getPaintConverter(), Color.DODGERBLUE, s -> s.highlightFill);

  static final List<CssMetaData<? extends Styleable, ?>> CSS_META_DATA_LIST;

  static {
    var styleables = new ArrayList<CssMetaData<? extends Styleable, ?>>(Path.getClassCssMetaData());
