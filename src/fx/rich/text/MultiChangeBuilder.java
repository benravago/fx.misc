 ...
 * {@link fx.rich.text.model.EditableStyledDocument} in one call via {@link #commit()}. <strong>Note: this builder
...
public class MultiChangeBuilder<PS, SEG, S> {
  ...
  final List<Replacement<PS, SEG, S>> list;
  ...
  MultiChangeBuilder(GenericStyledArea<PS, SEG, S> area, List<Replacement<PS, SEG, S>> list) {
    this.area = area;
    this.list = list;
  }
  ...
  public MultiChangeBuilder<PS, SEG, S> insertText(int paragraphIndex, int columnPosition, String text) {
    var index = area.getAbsolutePosition(paragraphIndex, columnPosition);
  ...
  public MultiChangeBuilder<PS, SEG, S> insert(int paragraphIndex, int columnPosition, StyledDocument<PS, SEG, S> document) {
    var pos = area.getAbsolutePosition(paragraphIndex, columnPosition);
  ...
  public MultiChangeBuilder<PS, SEG, S> deleteText(int startParagraph, int startColumn, int endParagraph, int endColumn) {
    var start = area.getAbsolutePosition(startParagraph, startColumn);
    var end = area.getAbsolutePosition(endParagraph, endColumn);
    return replaceText(start, end, "");
  }
  ...
  public MultiChangeBuilder<PS, SEG, S> replaceText(int start, int end, String text) {
    return relativeReplace(start, end, ReadOnlyStyledDocument.fromString(text, area.getInitialParagraphStyle(), area.getInitialTextStyle(), area.getSegOps()));
  }
  ...
  public MultiChangeBuilder<PS, SEG, S> replaceTextAbsolutely(int start, int end, String text) {
    return absoluteReplace(start, end, ReadOnlyStyledDocument.fromString(text, area.getInitialParagraphStyle(), area.getInitialTextStyle(), area.getSegOps()));
  }
  ...
  MultiChangeBuilder<PS, SEG, S> relativeReplace(int start, int end, ReadOnlyStyledDocument<PS, SEG, S> replacement) {
    if (list.isEmpty()) {
  ...
  MultiChangeBuilder<PS, SEG, S> absoluteReplace(int start, int end, ReadOnlyStyledDocument<PS, SEG, S> replacement) {
