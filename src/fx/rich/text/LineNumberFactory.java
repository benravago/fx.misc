...
public class LineNumberFactory<PS> implements IntFunction<Node> {
  ...
  static final Insets DEFAULT_INSETS = new Insets(0.0, 5.0, 0.0, 5.0);
  static final Paint DEFAULT_TEXT_FILL = Color.web("#666");
  static final Font DEFAULT_FONT = Font.font("monospace", FontPosture.ITALIC, 13);
  static final Font DEFAULT_FOLD_FONT = Font.font("monospace", FontWeight.BOLD, 13);
  static final Background DEFAULT_BACKGROUND = new Background(new BackgroundFill(Color.web("#ddd"), null, null));
  ...
  public static <PS> IntFunction<Node> get(GenericStyledArea<PS, ?, ?> area, IntFunction<String> format) {
    if (area instanceof StyleClassedTextArea) {
      var classArea = (StyleClassedTextArea) area;
      ...
      var inlineArea = (InlineCssTextArea) area;
  ...
  final Val<Integer> nParagraphs;
  final IntFunction<String> format;
  final GenericStyledArea<PS, ?, ?> area;
  final UnaryOperator<PS> removeFoldStyle;
  final Predicate<PS> isFoldedCheck;

  LineNumberFactory(GenericStyledArea<PS, ?, ?> area, IntFunction<String> format, Predicate<PS> isFolded, UnaryOperator<PS> removeFoldStyle) {
    ...
    if (isFoldedCheck != null) {
      area.getParagraphs().sizeProperty().addListener((ob, ov, nv) -> {
        Platform.runLater( nv <= ov ? () -> deleteParagraphCheck() : () -> insertParagraphCheck() );
      });
    }
  }
  ...
  public Node apply(int idx) {
    var formatted = nParagraphs.map(n -> format(idx + 1, n));
    ...
    if (isFoldedCheck != null) {
      ...
      if (area.getParagraphs().size() > idx + 1) {
        if (isFoldedCheck.test(area.getParagraph(idx + 1).getParagraphStyle()) && !isFoldedCheck.test(area.getParagraph(idx).getParagraphStyle())) {
          foldIndicator.setOnMouseClicked(ME -> area.unfoldParagraphs(idx, isFoldedCheck, removeFoldStyle));
