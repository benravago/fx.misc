...
public class SubscribeableContentsObsSet<E> extends AbstractSet<E> implements ObservableSet<E> {
  ...
  final List<Function<? super E, Subscription>> subscribers = new LinkedList<>();
  final List<SetChangeListener<? super E>> changeListeners = new LinkedList<>();
  final List<InvalidationListener> invalidationListeners = new LinkedList<>();
  ...
  public boolean add(E e) {
    ...
    var list = new ArrayList<Subscription>(1);
    subscribers.stream().map(f -> f.apply(e)).forEach(list::add);
    map.put(e, list);
    invalidateSet();
    fireElementAdded(e);
    return true;
  ...
  public Subscription addSubscriber(Function<? super E, Subscription> subscriber) {
    ...
    var keys = new ArrayList<E>(map.keySet());
    keys.forEach(key -> {
      var otherSubs = map.get(key);
      var sub = subscriber.apply(key);
      otherSubs.add(sub);
  ...
  void removeSubscriber(Function<? super E, Subscription> subscriber) {
  ...
  void fireElementAdded(E elem) {
    SetChangeListener.Change<E> change = new SetChangeListener.Change<E>(this) {
      ...
      @Override public E getElementAdded() { return elem; }
      @Override public E getElementRemoved() { return null; }
    };
    changeListeners.forEach(l -> l.onChanged(change));
  }

  void fireElementRemoved(E elem) {
    SetChangeListener.Change<E> change = new SetChangeListener.Change<E>(this) {
      @Override public boolean wasAdded() { return false; }
      @Override public boolean wasRemoved() { return true; }
      @Override public E getElementAdded() { return null; }
      @Override public E getElementRemoved() { return elem; }
    };
    changeListeners.forEach(l -> l.onChanged(change));
  }
