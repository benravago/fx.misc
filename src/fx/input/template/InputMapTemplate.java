 ...
 *
 *     final static InputMapTemplate&lt;? super Event&gt; BEHAVIOR;
 *
 *     static {
 *
...
public abstract class InputMapTemplate<S, E extends Event> {
  ...
  public static interface HandlerTemplateConsumer<S, E extends Event> {
    <F extends E> void accept(EventType<? extends F> t, InputHandlerTemplate<S, ? super F> h);
    static <S, E extends Event> HandlerTemplateConsumer<S, E> from(InputMap.HandlerConsumer<E> hc, S target) {
  ...
  InputHandlerTemplateMap<S, E> inputHandlerTemplates = null;
  ...
  static <S, E extends Event> InputMapTemplate<S, E> upCast(InputMapTemplate<S, ? extends E> imt) {
    ...
    var res = (InputMapTemplate<S, E>) imt;
    return res;
  }
  ...
  InputMapTemplate<S, E> postResult(Result checkedResult, BiConsumer<? super S, ? super E> postDesiredResult) {
  ...
  public static <S, T extends Event> InputMapTemplate<S, T> when(Predicate<? super S> condition, InputMapTemplate<S, T> imt) {
    return new InputMapTemplate<S, T>() {
      ...
      protected InputHandlerTemplateMap<S, T> getInputHandlerTemplateMap() {
        return imt.getInputHandlerTemplateMap()
          .map(h -> (s, evt) -> condition.test(s) ? h.process(s, evt) : Result.PROCEED);
      }
    };
  }
