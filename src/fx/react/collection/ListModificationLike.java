 ...
 *   <ul>
 *     <li>{@link ModifiedList} holds a reference to the observable list
 *     in which the modification occurred and the
 *     {@link ModifiedList#getAddedSubList()} method returns a sublist view
 *     of the original observable list. Such sublist is valid only until the
 *     next list modification.
 *     <li>{@link MaterializedModification} has its own copy of added
 *     elements ({@link MaterializedModification#getAdded()}), thus the
 *     validity of the list of added elements returned from {@code getAdded()}
 *     is not restricted.
 *     <li>{@link QuasiModification} does not provide a way to get the
 *     added elements directly, but has to be combined with the observable list
 *     to obtain {@linkplain ModifiedList}
 *     ({@link QuasiModification#instantiate(QuasiModification, ObservableList)})
 *     or {@linkplain MaterializedModification}
 *     ({@link QuasiModification#materialize(QuasiModification, ObservableList)}).
 *   </ul>
 *
