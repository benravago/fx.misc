...
public interface Val<T> extends ObservableValue<T>, Observable<Consumer<? super T>> {
  ...
  default T getOrSupply(Supplier<? extends T> defaultSupplier) {
    var res = getValue();
    return (res != null) ? res : defaultSupplier.get();
  }
   ...
   * map function.
   */
  default <U> Val<U> mapDynamic(ObservableValue<? extends Function<? super T, ? extends U>> f) {
   ...
   */
  default <U> Var<U> selectVar(Function<? super T, ? extends Property<U>> f) {
    return selectVar(this, f);
  }

  default <U> Var<U> selectVar(Function<? super T, ? extends Property<U>> f, U resetToOnUnbind) {
  ...
  static <T> Subscription observeChanges(ObservableValue<? extends T> obs, ChangeListener<? super T> listener) {
    if (obs instanceof Val<? extends T> val) {
      return val.observeChanges(listener);
    } else {
  ...
  static Subscription observeInvalidations(ObservableValue<?> obs, InvalidationListener listener) {
    obs.addListener(listener);
    return () -> obs.removeListener(listener);
  }
  ...
  static <T> Val<T> orElse(ObservableValue<? extends T> src, ObservableValue<? extends T> other) {
    return new OrElse<>(src, other);
  }

  static <T> Val<T> filter(ObservableValue<T> src, Predicate<? super T> p) {
    return map(src, t -> p.test(t) ? t : null);
  }

  static <T, U> Val<U> map(ObservableValue<T> src, Function<? super T, ? extends U> f) {
    return new MappedVal<>(src, f);
  }

  static <T, U> Val<U> mapDynamic(ObservableValue<T> src, ObservableValue<? extends Function<? super T, ? extends U>> f) {
    return combine(src, f, (t, fn) -> t == null || fn == null ? null : fn.apply(t));
  }

  static <T, U> Val<U> flatMap(ObservableValue<T> src, Function<? super T, ? extends ObservableValue<U>> f) {
    return new FlatMappedVal<>(src, f);
  }

  static <T, U> Var<U> selectVar(ObservableValue<T> src, Function<? super T, ? extends Property<U>> f) {
    return new FlatMappedVar<>(src, f);
  }

  static <T, U> Var<U> selectVar(ObservableValue<T> src, Function<? super T, ? extends Property<U>> f, U resetToOnUnbind) {
    return new FlatMappedVar<>(src, f, resetToOnUnbind);
  }

  static <T> Val<T> conditionOn(ObservableValue<T> obs, ObservableValue<Boolean> condition) {
  ...
  static <T> SuspendableVal<T> suspendable(ObservableValue<T> obs) {
    return (obs instanceof SuspendableVal<T> sv)
      ? sv : new SuspendableValWrapper<>((obs instanceof Val<T> v ? v : new ValWrapper<>(obs)));
  }
   ...
   * boundary values, given a fraction.
   */
  static <T> Val<T> animate(ObservableValue<T> obs, BiFunction<? super T, ? super T, Duration> duration, Interpolator<T> interpolator) {
   ...
   * boundary values, given a fraction.
   */
  static <T> Val<T> animate(ObservableValue<T> obs, Duration duration, Interpolator<T> interpolator) {
   ...
   * {@code T}.
   */
  static <T extends Interpolatable<T>> Val<T> animate(ObservableValue<T> obs, BiFunction<? super T, ? super T, Duration> duration) {
   ...
   * {@code T}.
   */
  static <T extends Interpolatable<T>> Val<T> animate(ObservableValue<T> obs, Duration duration) {
    return animate(obs, duration, Interpolator.get());
  }

  static <A, B, R> Val<R> combine(ObservableValue<A> _a, ObservableValue<B> _b, BiFunction<? super A, ? super B, ? extends R> f) {
    return create(() -> {
      var a=_a.getValue(); var b=_b.getValue();
      return (a != null && b != null) ? f.apply(a,b) : null;
      }, _a, _b);
  }

  interface TriFunction<A,B,C,R> { R apply(A a, B b, C c); }

  static <A, B, C, R> Val<R> combine(ObservableValue<A> _a, ObservableValue<B> _b, ObservableValue<C> _c, TriFunction<? super A, ? super B, ? super C,  ? extends R> f) {
    return create(() -> {
      var a=_a.getValue(); var b=_b.getValue(); var c=_c.getValue();
      return (a != null && b != null && c != null) ? f.apply(a,b,c) : null;
      }, _a, _b, _c);
  }

  interface QuadFunction<A,B,C,D,R> { R apply(A a, B b, C c, D d); }

  static <A, B, C, D, R> Val<R> combine(ObservableValue<A> _a, ObservableValue<B> _b, ObservableValue<C> _c, ObservableValue<D> _d, QuadFunction<? super A, ? super B, ? super C, ? super D,  ? extends R> f) {
    return create(() -> {
      var a=_a.getValue(); var b=_b.getValue(); var c=_c.getValue(); var d=_d.getValue();
      return (a != null && b != null && c != null && d != null) ? f.apply(a,b,c,d) : null;
      }, _a, _b, _c, _d);
  }

  interface QuinFunction<A,B,C,D,E,R> { R apply(A a, B b, C c, D d, E e); }

  static <A, B, C, D, E, R> Val<R> combine(ObservableValue<A> _a, ObservableValue<B> _b, ObservableValue<C> _c, ObservableValue<D> _d, ObservableValue<E> _e, QuinFunction<? super A, ? super B, ? super C, ? super D, ? super E, ? extends R> f) {
    return create(() -> {
      var a=_a.getValue(); var b=_b.getValue(); var c=_c.getValue(); var d=_d.getValue(); var e=_e.getValue();
      return (a != null && b != null && c != null && d != null && e != null) ? f.apply(a,b,c,d,e) : null;
      }, _a, _b, _c, _d, _e);
  }
  ...
  static Val<Boolean> showingProperty(Node node) {
    return Val.flatMap(node.sceneProperty(), Scene::windowProperty).flatMap(Window::showingProperty).orElseConst(false);
  }
