...
public interface Var<T> extends Val<T>, Property<T> {
  ...
  static <T> Var<T> fromVal(ObservableValue<T> obs, Consumer<T> setValue) {
    return (obs instanceof Val<T> v) ? v.asVar(setValue) : Val.wrap(obs).asVar(setValue);
  }
  ...
  static <T> SuspendableVar<T> suspendable(Property<T> p) {
    return (p instanceof SuspendableVar<T> sv)
      ? sv : new SuspendableVarWrapper<>( p instanceof Var<T> v ? v : new VarWrapper<T>(p) );
  }
