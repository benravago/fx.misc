...
public class FxTimer implements Timer {
  ...
  public static Timer runPeriodically(java.time.Duration interval, Runnable action) {
    var timer = createPeriodic(interval, action);
    timer.restart();
    return timer;
  }
  ...
  public static Timer runPeriodically0(java.time.Duration interval, Runnable action) {
    var timer = createPeriodic0(interval, action);
    timer.restart();
    return timer;
  }

  final Duration actionTime;
  final Timeline timeline;
  final Runnable action;

  long seq = 0;

  FxTimer(java.time.Duration actionTime, java.time.Duration period, Runnable action, int cycles) {
